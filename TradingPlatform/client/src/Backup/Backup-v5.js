...
function Content({ tab, setTab }) {
    ...
    const getDividends = useCallback(async (ticker) => {
        try {
            const response = await axios.get(`http://localhost:5000/api/dividends/${ticker}`);
    ...
    const getFinancials = useCallback(async (ticker) => {
        try {
            const response = await axios.get(`http://localhost:5000/api/financials/${ticker}`);
